...
uint256 hashGenesisBlock("0x0e7b83d15c0bab9cf34da519a871dac47ac4cae057af1cd90ee644253ab4c6a1");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Tocoin Signed Message:\n";
...
{
    // Tocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Tocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Tocoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Tocoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Tocoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe5a49e6ba3b59f9a483e8dfe83719467fa2f9fc85197a463a1bffb0c32e6097f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "News - 1st January 2014: The Large Hadron Collider (LHC) is set for a massive power boost.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x1aae37dbc55df8d693962d1aa0b140f089beb12645df753b87f4923a32ee4890"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Tocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TocoinMiner(CWallet *pwallet)
{
    printf("TocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tocoin-miner");
    ...
    try { loop {
        ...
        printf("Running TocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TocoinMiner, pwallet));
}
